...
| A flow's addressable version is `version_id`, and no `version` field exists | [`Flow` schema lines 30642–30652](https://github.com/windmill-labs/windmill/blob/v1.775.2/backend/windmill-api/openapi.yaml#L30642-L30652) declares `version_id: number`; [`runFlowByVersion` lines 12750–12762](https://github.com/windmill-labs/windmill/blob/v1.775.2/backend/windmill-api/openapi.yaml#L12750-L12762) takes the flow version ID; live response from Cloud `EE v1.779.0` on 2026-08-05 carried `version_id` and no `version` | High | Read `version_id` for pinning; reading `version` silently disables it and runs the latest version instead | Declared as `number`, so an integral float must also be accepted |
| An instance-scoped route answers a workspace-bound token with `401`, not `403` | Live probe of `app.windmill.dev` (`EE v1.779.0`) on 2026-08-05 with a throwaway token: `200` on `/api/w/{workspace}/{whoami,jobs/list,scripts/list,flows/list}` and `401` on `/api/workspaces/list`, `/api/users/whoami`, `/api/health/detailed`, `/api/workers/list`; token transport confirmed at [`extract_token` lines 523–528](https://github.com/windmill-labs/windmill/blob/v1.775.2/backend/windmill-api-auth/src/auth.rs#L523-L528) | High | Only a workspace-scoped route may read `401` as a bad credential; instance-scoped `401` degrades the capability and falls back to manual workspace entry (WMHA-0045) | Not reproduced against self-hosted CE/EE; the status may differ by deployment |
...
| --- | --- | --- | --- |
| PR-001 connection | `whoami`; optional workspace visibility/name | normal workspace member; restricted token needs `users:read`, plus `workspaces:read` when listing workspaces or resolving the name. On Windmill Cloud a workspace-bound token cannot list workspaces at all and is refused with `401`; onboarding must fall back to manual workspace entry rather than reject the token | Required |
